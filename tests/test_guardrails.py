...
spec = importlib.util.spec_from_file_location(
    "guardrails", ROOT / "scripts" / "guardrails.py"
)
...
def cfg(**over):
    c = dict(BASE_CFG)
    c.update(over)
    c["google_allowlist"] = set(c["google_allowlist"])
    c["meta_allowlist"] = set(c["meta_allowlist"])
    return c
...
def test_hard_blocked_action_denied():
    d, _ = g.evaluate(
        {"platform": "meta", "action": "delete_campaign", "account_id": "act_123"},
        cfg(),
        "ONAYLA | meta | act_123 | delete_campaign | 0",
    )
    assert d == "DENY"
...
def test_account_not_in_allowlist():
    d, _ = g.evaluate(
        ...
        cfg(),
        "ONAYLA | google | 9999999999 | create_campaign | 100",
    )
    assert d == "DENY"
...
def test_new_campaign_must_be_paused():
    d, _ = g.evaluate(
        ...
        cfg(),
        "ONAYLA | google | 1234567890 | create_campaign | 100",
    )
    assert d == "DENY"
    ...
    d, _ = g.evaluate(
        {
            ...
            "account_id": "1234567890",
            "daily_budget_try": 99999,
        },
        cfg(),
        "ONAYLA | google | 1234567890 | update_budget | 99999",
    )
    assert d == "DENY"
    ...
    d, _ = g.evaluate(
        {
            ...
            "account_id": "1234567890",
            "status": "PAUSED",
            "daily_budget_try": 148,
        },
        cfg(),
        "ONAYLA | google | 1234567890 | create_campaign | 148",
    )
    assert d == "ALLOW"
...
def test_create_campaign_no_entity_no_status_denied():
    ...
    d, _ = g.evaluate(
        ...
        cfg(),
        "ONAYLA | meta | act_123 | create_campaign | 100",
    )
    assert d == "DENY"
...
def test_create_keyword_no_status_allows():
    ...
    d, _ = g.evaluate(
        ...
        cfg(),
        "ONAYLA | google | 1234567890 | create_keyword | 0",
    )
    assert d == "ALLOW"
