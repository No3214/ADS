...
from kads.data_ext import WEEK_PLAN, PEAK_TIMES
...
def generate(days: int = 30, start: _dt.date | None = None) -> list[dict]:
    ...
    for i in range(days):
        ...
        for channel, concept, fmt in WEEK_PLAN.get(d.weekday(), []):
            rows.append({
                "tarih": d.isoformat(), "gün": _GUN[d.weekday()], "saat": PEAK_TIMES.get(channel, "11:00"),
                "kanal": channel, "konsept": concept, "format": fmt,
...
def write_csv(path: Path, rows: list[dict]) -> int:
    ...
    with path.open("w", encoding="utf-8-sig", newline="") as fh:
        w = csv.DictWriter(fh, fieldnames=["tarih", "gün", "saat", "kanal", "konsept", "format", "metin"])
