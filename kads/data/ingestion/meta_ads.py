...
def fetch_meta_campaigns() -> list[dict]:
    ...
    try:
        ...
        campaigns = account.get_campaigns(fields=[
            'id', 'name', 'status', 'daily_budget', 'bid_strategy'
        ])
        
        # Get insights (last 30 days)
        insights = account.get_insights(fields=[
            'campaign_id', 'spend', 'clicks', 'impressions', 'actions', 'action_values'
        ...
        for camp in campaigns:
            c_id = camp['id']
            metrics = insight_map.get(c_id, {'spend': 0.0, 'clicks': 0, 'impressions': 0, 'conversions': 0, 'revenue': 0.0})
            
            is_active = camp['status'] == 'ACTIVE'
            status = 'active' if is_active else 'paused'
            budget = float(camp.get('daily_budget', 0)) / 100.0  # Meta returns budget in cents
