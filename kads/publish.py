...
def to_postiz_rows(rows: list[dict]) -> list[dict]:
    ...
    for r in rows:
        out.append({
            "date": r["tarih"], "time": r["saat"],
            "channel": r["kanal"], "content": r["metin"],
            "media": r["format"], "concept": r["konsept"],
        })
    return out
...
def write_csv(path: Path, days: int = 30) -> int:
    ...
    with path.open("w", encoding="utf-8-sig", newline="") as fh:
        w = csv.DictWriter(fh, fieldnames=["date", "time", "channel", "content", "media", "concept"])
