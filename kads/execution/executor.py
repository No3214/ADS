...
def execute_action(action: FactActionJournal, db: Session) -> bool:
    ...
    if action.status != "approved":
        logger.warning(f"Action {action.action_id} cannot be executed because status is {action.status}")
        return False

    logger.info(f"Executing action {action.action_id}: {action.action_type} on {action.platform} {action.entity_type} {action.entity_id}")

    try:
        # Simulate execution / local state mutation
        if action.entity_type == "campaign":
            camp = db.query(DimCampaignState).filter_by(campaign_id=action.entity_id).first()
            if camp:
...
def rollback_action(action: FactActionJournal, db: Session) -> bool:
    ...
    if action.status != "executed":
        logger.warning(f"Action {action.action_id} cannot be rolled back because status is {action.status}")
        return False
    ...
    try:
        if action.entity_type == "campaign":
            camp = db.query(DimCampaignState).filter_by(campaign_id=action.entity_id).first()
            if camp:
                original_status = action.rollback_plan.get("status") or action.current_state.get("status")
                if original_status:
            ...
            camp = db.query(DimCampaignState).filter_by(campaign_id=action.entity_id).first()
            if camp:
                original_budget = action.rollback_plan.get("budget") or action.current_state.get("budget")
